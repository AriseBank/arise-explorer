...
node('arise-explorer-01'){
  lock(resource: "arise-explorer-01", inversePrecedence: true) {
    ...
    stage ('Start Arise ') {
      try {
        sh '''#!/bin/bash
        cp test/config_arise.json ~/arise-test/config.json
        bash ~/arise-test/arise.sh rebuild -f ~/arise-test/blockchain_explorer.db.gz
        '''
      } catch (err) {
        currentBuild.result = 'FAILURE'
        error('Stopping build, arise-core failed')
      }
    }

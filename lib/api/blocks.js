...
module.exports = function (app) {
    this.getBlockHeight = function (error, success) {
        request.get({
            url: app.get('arise address') + '/api/blocks/getHeight',
            json : true
    ...
    this.getBlockByHeight = function (height, error, success) {
        request.get({
            url: app.get('arise address') + '/api/blocks?height=' + height,
            json : true
        }, function (err, response, body) {
            if (err || response.statusCode !== 200) {
                return error({ success : false, error : (err || 'Response was unsuccessful') });
        ...
        this.getBlockHeight(
            ...
            function (data) {
                if (data.success === true) {
                    request.get({
                        url : app.get('arise address') + '/api/blocks?orderBy=height:desc&limit=20&offset=' + blocks.offset(n),
                        json : true
                    }, function (err, response, body) {
                        if (err || response.statusCode !== 200) {
                            return error({ success : false, error : (err || 'Response was unsuccessful') });
    ...
    function Block () {
        this.getBlock = function (blockId, height, cb) {
            request.get({
                url : app.get('arise address') + '/api/blocks/get?id=' + blockId,
                json : true
            }, function (err, response, body) {
                if (err || response.statusCode !== 200) {
                    return cb(err || 'Response was unsuccessful');
    ...
    this.getBlockStatus = function (error, success) {
        request.get({
            url : app.get('arise address') + '/api/blocks/getStatus',
            json : true
        }, function (err, response, body) {
            if (err || response.statusCode !== 200) {
                return error({ success : false, error : (err || 'Response was unsuccessful') });

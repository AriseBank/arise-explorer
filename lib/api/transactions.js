...
module.exports = function (app) {
    this.getTransaction = function (transactionId, error, success, url) {
        ...
        async.waterfall([
            function (waterCb) {
                request.get({
                    url : app.get('arise address') + url + transactionId,
                    json : true
    ...
    function Transaction () {
        this.getDelegate = function (forger, cb) {
            request.get({
                url : app.get('arise address') + '/api/delegates/get?publicKey=' + forger.publicKey,
                json : true
            }, function (err, response, body) {
                if (err || response.statusCode !== 200) {
    ...
    this.getUnconfirmedTransactions = function (error, success, transactions) {
        async.waterfall([
            function (cb) {
                request.get({
                    url : app.get('arise address') + '/api/transactions/unconfirmed',
                    json : true
                }, function (err, response, body) {
                    if (err || response.statusCode !== 200) {
                        return error({ success : false, error : (err || 'Response was unsuccessful') });
    ...
    this.getLastTransactions = function (error, success) {
        request.get({
            url : app.get('arise address') + '/api/transactions?orderBy=timestamp:desc&limit=20',
            json : true
        }, function (err, response, body) {
            if (err || response.statusCode !== 200) {
                return error({ success : false, error : (err || 'Response was unsuccessful') });
        ...
        async.waterfall([
            function (cb) {
                ...
                data.forEach(function (directionQuery, index) {
                    request.get({
                        url : app.get('arise address') + `/api/transactions?${directionQuery}`,
                        json : true
                    }, function (err, response, body) {
                        if (err || response.statusCode !== 200) {
    ...
    this.getTransactionsByBlock = function (query, error, success) {
        ...
        async.waterfall([
            function (cb) {
                request.get({
                    url : app.get('arise address') + '/api/transactions?blockId=' + query.blockId + '&orderBy=timestamp:desc' + '&offset=' + param(query.offset, 0) + '&limit=' + param(query.limit, 100),
                    json : true
                }, function (err, response, body) {
                    if (err || response.statusCode !== 200) {
                        return error({ success : false, error : (err || 'Response was unsuccessful') });
    ...
    var getSenderDelegate = function (transaction, cb, tmpDelegateCache) {
        ...
        request.get({
            url  : app.get ('arise address') + '/api/delegates/get?publicKey=' + transaction.senderPublicKey,
            json : true
        }, function (err, response, body) {
            if (err || response.statusCode !== 200) {
                transaction.senderDelegate = null;
    ...
    var getRecipientPublicKey = function (transaction, cb, tmpDelegateCache) {
        ...
        request.get ({
            url  : app.get ('arise address') + '/api/accounts/getPublicKey?address=' + transaction.recipientId,
            json : true
        }, function (err, response, body) {
            if (err || response.statusCode !== 200) {
                transaction.recipientPublicKey = null;
    ...
    var getRecipientDelegate = function (transaction, cb, tmpDelegateCache) {
        ...
        request.get ({
            url  : app.get ('arise address') + '/api/delegates/get?publicKey=' + transaction.recipientPublicKey,
            json : true
        }, function (err, response, body) {
            if (err || response.statusCode !== 200) {
                transaction.recipientDelegate = null;
